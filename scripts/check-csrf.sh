...
if [[ -z "${SESSION_COOKIE}" ]]; then
  echo "Usage: $0 <base_url> <session_cookie_value>"
  echo "Example: $0 https://your-api.example.com 'plaidhealth.sid=...'"
  exit 1
fi
...
STATUS_NO_TOKEN="$(curl -s -o /dev/null -w "%{http_code}" -X DELETE -H "Cookie: ${SESSION_COOKIE}" "${BASE_URL}/api/epic-connections/does-not-matter")"
...
STATUS_WITH_TOKEN="$(curl -s -o /dev/null -w "%{http_code}" -X DELETE \
  ...
  "${BASE_URL}/api/epic-connections/does-not-matter")"
